    ...
    <main className="w-full">
      ...
      <section className="relative w-full h-64 sm:h-[500px] overflow-hidden group">
        ...
        <div className="absolute top-1/2 left-0 right-0 flex justify-between items-center px-4 transform -translate-y-1/2">
          <button
            onClick={handlePrev}
            className="bg-black/50 hover:bg-black/70 p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition"
          >
        ...
        {packages.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {packages.map((pkg, index) => (
              <div
                key={index}
                className="bg-white rounded-xl shadow-md hover:shadow-2xl transform hover:scale-[1.02] transition-all duration-300 border overflow-hidden animate-fadeIn"
              >
                {pkg.photos.length > 0 && (
                  <img
                    src={pkg.photos[0]}
